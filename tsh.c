...
/* My own extra helper methods/vars */
void listjob(struct job_t *job);
void listjobint(struct job_t *job, int sig);
void listjobstop(struct job_t *job, int sig);
...
void eval(char *cmdline) 
{
  ...
  if (!strcmp(argv1[0], "quit") || !strcmp(argv1[0], "jobs") || !strcmp(argv1[0], "bg") || !strcmp(argv1[0], "fg")) {
    builtin_cmd(argv1);
  }
  else {
  
  // must block SIGCHLD (possibly also SIGINT and SIGSTP)
 ...
 // sigaddset(&sigs, SIGTSTP);
  ...
  if (pid == 0) {
    ...
    int chldres = 1;
    chldres = execve(argv1[0], argv1, environ);
...
int builtin_cmd(char **argv) 
{
  ...
  // TODO: Only print background jobs?
  if (!strcmp(cmd, "jobs")) {
     listjobs(jobs);
     return 1; // return?
  }
...
void sigchld_handler(int sig) 
{
  ...
  // What is the correct pid? use fg job?
  
  int status;

  int res = 0;

  while((res = waitpid(-1, &status, WNOHANG | WUNTRACED)) > 0) {
     if (WIFSTOPPED(status) != 0) {
       struct job_t * stoppedJob = getjobpid(jobs, res);
       kill(getpid(), SIGTSTP);
       stoppedJob->state = ST;

       //if (getpid() == res)
         //listjobstop(stoppedJob, WSTOPSIG(status));
     }
       ...
       deletejob(jobs, res);
       //if (getpid() == res)
        // listjobint(stoppedJob, WTERMSIG(status));
...
/* Print a single job, I use this after running a background job */
void listjob(struct job_t *job) {
...
/* Print a job, and which signal terminated it */
void listjobint(struct job_t *job, int sig) {
      sio_puts("Job [");
      sio_putl(job->jid);
      sio_puts("] (");
      ...
      sio_putl(sig);
      sio_puts("\n");
      //printf("Job [%d] (%d) terminated by %d", job->jid, job->pid, sig);
